...
from finance_graph.common import config,write_json
from pathlib import Path
os.environ['HF_HUB_OFFLINE']='1'
os.environ['TOKENIZERS_PARALLELISM']='false'
from mlx_lm import load,batch_generate
import mlx.core as mx
parser=argparse.ArgumentParser(description='Local MLX pilot (requires the local extra)');parser.add_argument('--config');args=parser.parse_args()
cfg=config(args.config,backend='local_mlx');t=time.time()
...
write_json(Path(cfg['artifact_dir'])/'pilot.json',{'texts':r.texts,'stats':vars(r.stats),'model':cfg['model_id']})
