...
def main():
 ...
 results=read_jsonl(out/'evaluation/results.jsonl');gold={q['id']:q for q in load_json(cfg['benchmark_path'])};metrics=read_jsonl(out/'evaluation/retrieval_metrics.jsonl');byid={m['id']:m for m in metrics};g=Graph(out/'graph.sqlite');rows=[]
 for r in results:
 ...
 statements=collections.Counter(digest(f['properties']['statement']) for f in g.facts());duplicates=sum(v-1 for v in statements.values());review=load_json(ROOT/'docs/manual_error_review.json');manual=[]
 for case in review:
